...
if (authItem && link && hasSupabaseConfig() && supabase) {
  const apply = async () => {
    ...
    if (session?.user) {
      ...
      if (isMobile) {
        // En móvil, sin dropdown: CTA directo y limpio
      ...
      menu.hidden = true;
      menu.innerHTML =         '<a href="/plantillas" class="nav__auth-dropdown-link">Mis plantillas</a>' +
      ...
      link.addEventListener('click', (e) => { e.preventDefault(); menu.hidden ? openMenu() : closeMenu(); });
      document.addEventListener('click', (e) => { if (!authItem.contains(e.target)) closeMenu(); });
      ...
      const menu = authItem.querySelector('.nav__auth-dropdown');
      if (menu) menu.remove();
    }
  };
